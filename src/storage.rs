// use soroban_sdk::{contracttype, Env, Bytes, Map, Symbol, Vec};
// use soroban_ecs::prelude::World;
...
// #[contracttype]
// #[derive(Clone, Debug)]
// pub struct WorldSerialization {
//     entities: Vec<u64>,
//     components: Map<u64, Map<Symbol, Bytes>>,
//     resources: Map<Symbol, Bytes>,
//     events: Map<Symbol, Vec<Bytes>>,
// }

// impl WorldSerialization {
//     pub fn from_world(world: &World, env: &Env) -> Self {
//         let mut entities = Vec::new(env);
//         for entity_id in world.query_entities(&[]).iter() {
//             entities.push_back(entity_id.id());
//         }

//         let mut components = Map::new(env);
//         for entity_id in entities.iter() {
//             let mut entity_components = Map::new(env);
//             if let Some(comp_map) = world.get_components(&EntityId::new(entity_id, 0)) {
//                 for (symbol, component) in comp_map.iter() {
//                     entity_components.set(symbol, component.data().clone());
//                 }
//             }
//             components.set(entity_id, entity_components);
//         }

//         let mut resources = Map::new(env);
//         for (symbol, resource) in world.get_all_resources() {
//             resources.set(symbol, resource.data().clone());
//         }

//         let mut events = Map::new(env);
//         for event_type in world.get_all_event_types() {
//             let mut event_vec = Vec::new(env);
//             for event in world.get_events(&event_type).iter() {
//                 event_vec.push_back(event.data().clone());
//             }
//             events.set(event_type, event_vec);
//         }
...
//     pub fn to_world(&self, env: &Env) -> World {
//         let mut world = World::new();
//         for entity_id in self.entities.iter() {
//             world.spawn_empty_with_id(EntityId::new(entity_id, 0));
//         }

//         for (entity_id, comp_map) in self.components.iter() {
//             for (symbol, data) in comp_map.iter() {
//                 let component = Component::new(symbol, data);
//                 world.add_component_to_entity(EntityId::new(entity_id, 0), component);
//             }
//         }

//         for (symbol, data) in self.resources.iter() {
//             let resource = Resource::new(symbol, data);
//             world.add_resource(resource);
//         }

//         for (event_type, event_vec) in self.events.iter() {
//             for event_data in event_vec.iter() {
//                 world.send_event(Event::new(event_type.clone(), event_data));
//             }
//         }
...
// pub fn set_world(env: &Env, world: &World) {
//     let key = DataKey::World;
//     let serialization = WorldSerialization::from_world(world, env);
//     env.storage().instance().set(&key, &serialization);
// }

// pub fn get_world(env: &Env) -> Option<World> {
//     let key = DataKey::World;
//     if let Some(serialization) = env.storage().instance().get::<_, WorldSerialization>(&key) {
//         Some(serialization.to_world(env))
//     } else {
//         None
//     }
// }
...
//         GamePosition (
//             pos.0.saturating_add(dx.try_into().unwrap()),
//             pos.1.saturating_add(dy.try_into().unwrap()),
//         )
//     }
// } 
...
//         let health = 100_u32;
//         let entity_data: (u32, u32, u32, u32) = (entity_id, position.0, position.1, health);
